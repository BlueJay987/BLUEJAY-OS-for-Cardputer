...
// I'm building this exclusevly for CardPuter, so no need for these
/*
...
*/
  ...
  #define BITMAP M5Cardputer.Display.drawBmp(BRUCEMatrix, 44190)
...
int spammenu_size = sizeof(spammenu) / sizeof(MENU);

void spammenu_setup() {
  cursor = 0;
  rstOverride = true;
  drawmenu(spammenu, spammenu_size);
  delay(500); // Prevent switching after menu loads up
}
...
void spammenu_loop() {
  ...
  if (check_select_press()) {
    rstOverride = false;
    isSwitching = true;
    int option = spammenu[cursor].command;
    switch(option) {
      case 1:
        rstOverride = false;
        isSwitching = true;
...
void bootScreen(){
  ...
  DISP.println("Modified by BlueJay");
  delay(1500);
  DISP.setTextColor(FGCOLOR, BGCOLOR);
  DISP.println(" ");
  DISP.println(" Based off M5-BRUCE");
  while(true){
    M5Cardputer.update();
...
void changelog(){
  ...
  switch (version) {
    case 1:
      DISP.println(v1_1log);
      break;
    case 2:
      DISP.println(v1_2log);
      break;
    case 3:
      DISP.println(v1_3log);
      break;
...
void setup() {
  ...
  if(check_next_press()){
    ...
    if(EEPROM.read(0) > 3 || EEPROM.read(1) > 240 || EEPROM.read(2) > 100 || EEPROM.read(3) > 1 || EEPROM.read(4) > 19 || EEPROM.read(5) > 19) {
      ...
      EEPROM.write(2, 50);  // 50% brightness
      EEPROM.write(3, 0);    // TVBG NA Region
      EEPROM.write(4, 10);   // FGColor Blue
